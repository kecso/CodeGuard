...
def test_full_run_with_fake_model(tmp_path: Path) -> None:
    ...
    assert result.repos[0].report_path == "reports/audit-20260828T000000Z.md"
    assert created and created[0].prompts
    report = (tmp_path / "reports" / "audit-20260828T000000Z.md").read_text(
        encoding="utf-8"
    )
    ...
    latest = tmp_path / "reports" / "audit-latest-real.md"
    assert latest.is_file()
    checkout = tmp_path / "workspace" / "sample"
    assert not (checkout / "reports").exists()
    from git import Repo

    verify = tmp_path / "from-origin"
    Repo.clone_from(mirror, verify)
    assert not (verify / "reports").exists()


def test_dry_run_skips_model(tmp_path: Path) -> None:
    ...
    assert result.repos[0].status == "dry-run"
...
def test_same_commit_writes_empty_report(tmp_path: Path) -> None:
    ...
    assert len(created) == 1
    empty = (tmp_path / "reports" / "audit-20260828T000000Z.md").read_text(
        encoding="utf-8"
    )
    assert "No new findings" in empty
    assert (tmp_path / "reports" / "audit-20260827T000000Z.md").is_file()
...
def test_git_failure_is_isolated(tmp_path: Path) -> None:
    write_config(
        ...
        repositories=[
            {
                "name": "missing",
                "git_url": "/definitely/not/a/mirror.git",
                "branch": "main",
                "output_report_path": "reports/audit.md",
            }
        ],
...
def test_parser_flags() -> None:
    args = build_parser().parse_args(
        ["-c", "cfg.json", "--dry-run", "--force", "--repo", "a", "--repo", "b", "-v"]
    )
